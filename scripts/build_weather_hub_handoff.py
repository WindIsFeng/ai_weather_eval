...
def build_handoff(output_dir: Path = OUTPUT_DIR) -> dict[str, object]:
    ...
    for plan in plans:
        ...
        if plan["schedule_status"] != "matched":
            raise ValueError(f"unmatched initialization: {forecast_case_id}")
        ...
        init = _utc(plan["init_time"])
        reference = _utc(plan["reference_time"])
        target = _utc(plan["target_init_time"])
        if init.minute or init.second or init.microsecond or init.hour % STEP_HOURS:
            raise ValueError(f"initialization is not a six-hour UTC cycle: {forecast_case_id}")
        ...
        if not math.isclose(actual_hours, float(plan["actual_lead_hours"]), abs_tol=1e-6):
            raise ValueError(f"actual lead mismatch: {forecast_case_id}")
        if not math.isclose(offset_hours, float(plan["cycle_offset_hours"]), abs_tol=1e-6):
            raise ValueError(f"cycle offset mismatch: {forecast_case_id}")
        if abs(offset_hours) > 3:
            raise ValueError(f"cycle offset exceeds three hours: {forecast_case_id}")
        ...
        index_rows.append({
            ...
            "actual_lead_hours": plan["actual_lead_hours"],
            "cycle_offset_hours": plan["cycle_offset_hours"],
            "init_time": _iso(init),
            "forecast_hours": str(hours),
    ...
    manifest: dict[str, object] = {
        ...
        "rule": "forecast_hours = ceil((reference_time - init_time) / 6 hours) * 6 hours",
        ...
        "sources": {
            path.relative_to(ROOT).as_posix(): _sha256(path) for path in (EVENTS, PLANS)
        },
        "artifacts": {
